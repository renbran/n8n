...
export class NotificationsPage {
	...
	/**
	 * Closes ALL visible notifications on the screen (regardless of text).
	 * Fast execution with minimal waits.
	 * @param options Optional configuration
	 */
	async closeAllNotifications(options: { timeout?: number } = {}): Promise<number> {
		const { timeout = 1500 } = options;
		let closedCount = 0;

		try {
			const allNotifications = this.page.getByRole('alert');
			const count = await allNotifications.count();

			// Close all notifications rapidly
			for (let i = 0; i < count; i++) {
				try {
					const notification = allNotifications.nth(i);
					if (await notification.isVisible({ timeout: 200 })) {
						const closeBtn = notification.locator('.el-notification__closeBtn');
						await closeBtn.click({ timeout: 300 });
						closedCount++;
					}
			...
			// Brief wait for cleanup, don't block test execution
